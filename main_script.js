import * as _$ from "https://cdn.jsdelivr.net/npm/bijou.js@v7.1.2";
...
function run() {
	window.addEventListener("keydown", konami(() => settings()))
	...
	function updateHistory() {
		...
		history.pushState({}, `${inp.value} – Search`, `?q=${escape(inp.value)}&darkMode=${+document.documentElement.classList.contains("dark-mode")}&mode=${mode()}${document.documentElement.classList.contains("cool-mode") ? "&coolMode=1" : ""}`)
	}
		...
		(async () => {
			...
			function update() {
				...
				out.innerHTML += cache[url.toLowerCase()].map(i => ({
					...
					let scratch_re = /(?:http|https):\/\/scratch.mit.edu\/users\/([a-z0-9\-]+)/i
					let gist_re = /(?:http|https):\/\/gist.github.com\/([a-z0-9\-]+)\/([a-z0-9\-]+)/i
					let special_stuff = "";
					if (gist_re.test(i.link)){
						special_stuff = `<iframe width="100% height="400" src="data:text/html;charset=utf-8,<head><base target='_blank' /></head><body><script src='${i.link}.js'></script></body>">`
					}
					if (scratch_re.test(i.link)){
						special_stuff = `<iframe width="700" height="236" style="border: none !important;" src="https://6k11o.csb.app/?user=${escape(i.link.match(scratch_re)[1])}"></iframe>`
					}
	...
	window.onkeyup = (e) => {
		if (document.activeElement !== inp && e.key !== "A" && e.key !== "B") {
			inp.focus();
	...
	async function settings(){
		await alert({
			title: "Settings:",
			text: `<label><input type='checkbox' id='coolMode' onclick='document.documentElement.classList.toggle("cool-mode")'> Cool mode</label><br><label><input type='checkbox' id='darkMode' onclick='document.documentElement.classList.toggle("dark-mode")'> Dark mode</label><br><label><input type='checkbox' id='incognito' onclick='history.pushState({}, "Search engine", this.checked ? : "?incognito=1" : "")'> Incognito</label><br>`,
			buttontext: "Save"
		});
		updateHistory();
	}
